...
void Client::requestFile(int &fileSizeOut, std::string fileName)
{
    ...
    uint32_t frameSize = 0;//Needs to be uint on the server too!
    //Request file
    ...
    size_t size = recv(socketfd, buff, 1000, 0);
    if(size<0)
        Debug::error("Error, couldn't recv from server");
    //Calc the frameSize from 2 first received bytes
    frameSize = (buff[0]<<24) |(buff[1] << 16) | (buff[2] << 8) | buff [3];
    //Allocate another buffer to hold the current frame
    
    if((size_t)frameSize == size-2) //Everything was received in first recv
    {
...
void Client::requestFile(int &fileSizeOut, DataFrame fileName)
{
    //Size of the frame to be received, not including the framesize at the start of recv
